...
{
	Bucket *function_table_iter = function_table_tail ?
		function_table_tail->pListNext : embed_global_function_table.pListHead;
	Bucket *class_table_iter    = class_table_tail ?
		class_table_tail->pListNext : embed_global_class_table.pListHead;

	while (function_table_iter != NULL) {
		zend_op_array *op_array = (zend_op_array*)function_table_iter->pData;
		if (op_array->type == ZEND_USER_FUNCTION) {
			if (zend_hash_add(CG(function_table), function_table_iter->arKey,
					function_table_iter->nKeyLength, op_array, sizeof(zend_op_array),
					NULL) == FAILURE && function_table_iter->arKey[0] != '\0') {
				CG(in_compilation) = 1;
		...
		function_table_iter = function_table_iter->pListNext;
	}

	while (class_table_iter != NULL) {
		zend_class_entry **ce = (zend_class_entry**)class_table_iter->pData;
		if ((*ce)->type == ZEND_USER_CLASS) {
			if (zend_hash_add(CG(class_table), class_table_iter->arKey,
					class_table_iter->nKeyLength, ce, sizeof(zend_class_entry*),
					NULL) == FAILURE && class_table_iter->arKey[0] != '\0') {
				CG(in_compilation) = 1;
		...
		class_table_iter = class_table_iter->pListNext;
	}
}
